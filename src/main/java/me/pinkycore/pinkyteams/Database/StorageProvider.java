...
public interface StorageProvider {
    ...
    /** Migrates every player-name keyed record after a verified UUID name change. */
    default void renamePlayerIdentity(String oldName, String newName) {
        if (oldName == null || newName == null || oldName.equalsIgnoreCase(newName)) return;
        try (java.sql.Connection connection = getConnection()) {
            boolean autoCommit = connection.getAutoCommit();
            connection.setAutoCommit(false);
            try {
                String[] usernameTables = {"clan_users", "clan_invites", "player_stats"};
                for (String table : usernameTables) {
                    try (java.sql.PreparedStatement statement = connection.prepareStatement(
                        ...
                        // Some legacy schemas do not contain every optional table/column.
                    }
                }
                try (java.sql.PreparedStatement statement = connection.prepareStatement(
                    "UPDATE clans SET leader = CASE WHEN leader = ? THEN ? ELSE leader END, " +
                        "founder = CASE WHEN founder = ? THEN ? ELSE founder END")) {
                    statement.setString(1, oldName); statement.setString(2, newName);
                    statement.setString(3, oldName); statement.setString(4, newName);
                    statement.executeUpdate();
                }
                connection.commit();
            ...
            throw new IllegalStateException("Could not migrate player identity", e);
        }
    }
    ...
    void setFriendlyFireAlliesEnabled(String clanName, boolean enabled);
