...
public class YamlStorageProvider extends AbstractStorageProvider {
    ...
    @Override
    public void renamePlayerIdentity(String oldName, String newName) {
        if (oldName == null || newName == null || oldName.equalsIgnoreCase(newName)) return;
        lock.writeLock().lock();
        try {
            ConfigurationSection users = data.getConfigurationSection("clan_users");
            if (users != null) {
                for (String clan : users.getKeys(false)) moveYamlValue("clan_users." + clan, oldName, newName);
            }
            ConfigurationSection invites = data.getConfigurationSection("clan_invites");
            if (invites != null) {
                for (String clan : invites.getKeys(false)) moveYamlValue("clan_invites." + clan, oldName, newName);
            }
            moveYamlValue("player_stats", oldName, newName);
            ConfigurationSection clans = data.getConfigurationSection("clans");
            if (clans != null) {
                for (String clan : clans.getKeys(false)) {
                    String base = "clans." + clan;
                    if (oldName.equalsIgnoreCase(data.getString(base + ".leader"))) data.set(base + ".leader", newName);
                    if (oldName.equalsIgnoreCase(data.getString(base + ".founder"))) data.set(base + ".founder", newName);
                }
            }
            saveData();
            reloadCache();
    ...
    private void moveYamlValue(String parent, String oldName, String newName) {
        ConfigurationSection section = data.getConfigurationSection(parent);
        if (section == null) return;
        for (String key : section.getKeys(false)) {
            if (key.equalsIgnoreCase(oldName)) {
                data.set(parent + "." + newName, data.get(parent + "." + key));
                data.set(parent + "." + key, null);
                return;
            }
        }
    }
    ...
    protected void setFriendlyFireAlliesEnabledImpl(String clanName, boolean enabled) throws Exception {
        lock.writeLock().lock();
        try {
            data.set("friendlyfire_allies." + clanName, enabled);
            saveData();
        } finally {
            lock.writeLock().unlock();
        }
