...
public abstract class AbstractStorageProvider implements StorageProvider {
    ...
    protected final PinkyTeams plugin;
    ...
    @Override
    public void setFriendlyFireAlliesEnabled(String clanName, boolean enabled) {
        if (clanName == null || clanName.trim().isEmpty()) return;
        try {
            setFriendlyFireAlliesEnabledImpl(clanName, enabled);
        } catch (Exception e) {
            plugin.getLogger().severe("Error updating ally friendly fire for " + clanName + ": " + e.getMessage());
        }
    }
    ...
    protected abstract void setFriendlyFireAlliesEnabledImpl(String clanName, boolean enabled) throws Exception;
