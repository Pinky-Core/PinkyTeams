...
public class Econo {
    ...
    public Econo(PinkyTeams plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean deposit(OfflinePlayer player, double amount) {
        if (player == null || !Double.isFinite(amount) || amount <= 0) return false;
        String selected = getSystem();
        if ("vault".equalsIgnoreCase(selected)) {
    ...
    private boolean depositVault(OfflinePlayer player, double amount) {
        if (vaultProvider == null || vaultDeposit == null) {
            return false;
        }
        try {
            return transactionSucceeded(vaultDeposit.invoke(vaultProvider, player, amount));
        } catch (Exception ignored) {
            return false;
        }
    }

    private boolean withdrawVault(OfflinePlayer player, double amount) {
        if (vaultProvider == null || vaultWithdraw == null) {
            return false;
        }
        try {
            return transactionSucceeded(vaultWithdraw.invoke(vaultProvider, player, amount));
        } catch (Exception ignored) {
            return false;
        }
    }
