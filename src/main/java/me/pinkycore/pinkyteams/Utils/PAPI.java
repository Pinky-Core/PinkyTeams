...
public class PAPI extends PlaceholderExpansion implements Relational {
    ...
    public static final Set<String> SUPPORTED_PLACEHOLDERS = Set.of(
        "prefix","player_money","clan_leader","clan_founder","clan_name","clan_tag","clan_money",
        "clan_kills","clan_deaths","clan_membercount","clan_membercount_online","clan_membercount_offline",
        "clan_war_opponent","clan_war_score","clan_war_opponent_score","clan_war_time_left",
        "season_name","season_active","season_time_left"
    );
    ...
    public String onPlaceholderRequest(Player player, @NotNull String identifier) {
        ...
        if (normalized.equals("season_name")) return plugin.getClanSeasonService().name();
        if (normalized.equals("season_active")) return String.valueOf(plugin.getClanSeasonService().isActive());
        if (normalized.equals("season_time_left")) return String.valueOf(plugin.getClanSeasonService().timeLeftSeconds());

        if (normalized.equals("prefix")) {
            return PinkyTeams.prefix;
        }

        if (player == null) return getNoClanPlaceholder();

        Econo econ = plugin.getEcon();
        if (normalized.equals("player_money")) {
        ...
        switch (normalized) {
            ...
            case "clan_war_opponent":
                return plugin.getClanWarService().findWar(clanName)
                    .map(war -> war.opponentOf(clanName)).orElse(getNoClanPlaceholder());
            case "clan_war_score":
                return plugin.getClanWarService().findWar(clanName)
                    .map(war -> String.valueOf(war.scoreFor(clanName))).orElse("0");
            case "clan_war_opponent_score":
                return plugin.getClanWarService().findWar(clanName)
                    .map(war -> String.valueOf(war.scoreFor(war.opponentOf(clanName)))).orElse("0");
            case "clan_war_time_left":
                return plugin.getClanWarService().findWar(clanName)
                    .map(war -> String.valueOf(Math.max(0L, (war.endsAt() - System.currentTimeMillis()) / 1000L))).orElse("0");

            default:
                return "&d&lᴘɪɴᴋʏᴛᴇᴀᴍꜱ";
        }
    }
     ...
     * Use %rel_pinkyteams_nametag_obfuscate_prefix% and %rel_pinkyteams_nametag_obfuscate_reset%.
     */
    @Override
    public String onPlaceholderRequest(Player viewer, Player target, @NotNull String identifier) {
        ...
        String bypass = config.getString("nametag-privacy.bypass-permission", "pinkyteams.bypass.nametag");
    ...
    public void registerPlaceholders() {
        if (!register()) {
            plugin.getLogger().warning("Failed to register PinkyTeams placeholders.");
        } else {
            plugin.getLogger().info("PinkyTeams placeholders registered!");
        }
    }
